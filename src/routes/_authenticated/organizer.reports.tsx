...
import { KpiCard, PageHeader, Panel, ProBadge } from "@/components/organizer/ui";
import { caravans, fcfa, monthlySeries, pct } from "@/lib/organizer";

export const Route = createFileRoute("/organizer/reports")({
  head: () => ({
    meta: [
...
function ReportsPage() {
  ...
  const seats = caravans.reduce((a, c) => a + c.booked, 0);
          ...
          <table className="w-full min-w-[640px] text-sm">
            <thead>
              <tr className="border-b border-border text-left text-[11px] uppercase tracking-wide text-muted-foreground">
                ...
                <th className="px-5 py-3 font-semibold">Commission (3%)</th>
                <th className="px-5 py-3 font-semibold">Net</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {caravans.map((c) => {
                const commission = Math.round(c.revenue * 0.03);
                return (
